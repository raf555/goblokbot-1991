    ...
    if (data && !data.uploaded && data.expire > Date.now()) {
        try {
            ...
            if (data.imgbb) {
                ...
                let file = await discordCDN.createFile(data.name, buffer);
                let res = await discordCDN.uploadFile(file, event.source.userId);
